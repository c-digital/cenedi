    ...
    public function invoice($id = '')
    {
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                foreach ($invoice_data['newitems'] as $item) {
                    $id_item = $item['id_item'];
                    $quantity = $item['qty'];
                    $query = $this->db->query("SELECT * FROM tblitems WHERE id = $id_item");
                    $current_item = $query->row();

                    if ($current_item->treatment == 'Si') {
                        $id_client = $invoice_data['clientid'];
                        $realized = json_encode(array());

                        $this->db->query(
                            "
                                INSERT INTO
                                    tblparts
                                        (id_client, id_item, id_sale, quantity, realized)
                                    VALUES
                                        ('$id_client', '$id_item', '$id', '$quantity', '$realized');
                            "
                        );
                    }
                }
